...
### Option 1: GleanDebugger Component (Recommended for Next.js)

For Next.js App Router, use dynamic import to prevent SSR hydration issues:
...
export function Providers({ children }: { children: React.ReactNode }) {
  return (
    <>
      {children}
      <GleanDebugger environment="development" user={{ id: 'user_123', role: 'admin' }} />
    </>
  );
}
```
...
### Option 2: useLogRecorder + DebugPanel (Advanced)

For custom configurations or non-Next.js frameworks:
