...
For Next.js apps, use the `GleanDebugger` component with dynamic import:
...
Then configure the `useLogRecorder` for manual uploads if needed:
...
function useDebugLogs() {
  ...
  return { ...recorder, handleSupportRequest };
}
```
